...
async function run() {
  console.log('Iniciando sincronización total desde Google Sheets...');
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (id) {
        ...
        if (lead?.adId && !lead.campaignName) {
           await enrichLeadAdData(lead.id, lead.adId);
        }
      }
    }
  }
  console.log(`${leadCount} leads procesados.`);
  ...
  console.log(`${saleCount} ventas procesadas.`);
}
